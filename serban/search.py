...
class Sampler(object):
    ...
    def sample(self, *args, **kwargs):
        ...
        if not self.model.reset_utterance_decoder_at_end_of_utterance:
            ...
            ran_vector = self.model.rng.normal(
                size=(context.shape[0], n_samples, self.model.latent_gaussian_per_utterance_dim)).astype('float32')
        ...
        ran_vectors = self.model.rng.normal(size=(n_samples, self.model.latent_gaussian_per_utterance_dim)).astype(
            'float32')
                ...
                for idx in range(context.shape[1]):
                    ...
                    for eos_index in eos_indices:
                        reversed_context[(prev_eos_index + 2):eos_index, idx] = (reversed_context[
                                                                                 (prev_eos_index + 2):eos_index, idx])[
                                                                                ::-1]
