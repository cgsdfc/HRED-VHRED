...
Usage example for MT embeddings:
    ...
    python convert_wordemb_dict2emb_matrix.py tests/data/ttrain.dict.pkl WordEmb/D_german_50k_500k_168h.pkl --emb_dim 15 temb_pretrained
    python convert_wordemb_dict2emb_matrix.py Data/Training.dict.pkl WordEmb/D_german_50k_500k_168h.pkl --apply_spelling_corrections --emb_dim 300 OutMat

Usage example for Word2Vec embeddings:

    python convert_wordemb_dict2emb_matrix.py Data/Training.dict.pkl WordEmb/GoogleNews-vectors-negative300.bin --apply_spelling_corrections --emb_dim 300 Word2Vec_WordEmb
...
# Takes a word as input and creates a set of one-character changes to the word
# (splits, transposes, replaces, insert etc.).
def edits1(word):
...
if __name__ == '__main__':
    ...
    non_word_tokens = ['<s>', '</s>', '<t>', '</t>', '<unk>', '.', ',', '``', '\'\'', '[', ']', '`', '-', '--', '\'',
                       '<pause>', '<first_speaker>', '<second_speaker>', '<third_speaker>', '<minor_speaker>',
                       '<voice_over>', '<off_screen>', '</d>']

    print('The following non-word tokens will not be extracted from the pretrained embeddings: ', non_word_tokens)

    parser = argparse.ArgumentParser()
    parser.add_argument("model_dictionary", type=str, help="Model dictionary generated by convert_text2dict.py")
    parser.add_argument("embedding_dictionary", type=str,
                        help="Python vocabulary (pkl file) "
                             "which contains a dictionary for every word, or Word2vec word embedding (bin file)")

    parser.add_argument("--emb_dim",
                        type=int,
                        default=300,
                        help="Dimensionality of the generated word embedding matrix. "
                             "PCA is performed to reduce dimensionality to this size.")

    parser.add_argument("--std_dev", type=float, default=0.01,
                        help="Standard deviation of the produced word embedding.")

    parser.add_argument("--apply_spelling_corrections",
                        action='store_true',
                        help="If true, will apply spelling corrections to all words not found in"
                             " the pretrained word embedding dictionary. These corrections are based"
                             " on regex expressions and the enchant spelling corrector.")
    ...
    rand_mat = NormalInit(rng, unique_words_missing, emb_dim)
    for i in range(unique_words_missing):
        W_emb[unique_word_indices_left_out[i], :] = rand_mat[i, :]

    # Create mask matrix, which represents word embeddings not pretrained
    W_emb_non_pretrained_mask = numpy.zeros((i_dim, emb_dim))
    for i in range(unique_words_missing):
        W_emb_non_pretrained_mask[unique_word_indices_left_out[i], :] = 1

    safe_pickle([W_emb, W_emb_non_pretrained_mask], args.output_matrix + ".pkl")
